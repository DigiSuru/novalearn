<template>
  <div class="min-h-screen flex bg-white font-sans selection:bg-blue-600 selection:text-white">
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-12 relative z-10 bg-white">
      <!-- Mobile/Desktop logo -->
      ...
      <div class="max-w-md w-full mt-12 lg:mt-0">
        <div class="mb-10">
          <h2 class="text-4xl font-black text-gray-900 tracking-tight mb-2">Join the Elite</h2>
          <p class="text-gray-500 font-medium">Create your free account to access verified learning materials.</p>
        </div>
        ...
        <form @submit.prevent="handleRegister" class="space-y-5">
          <div>
            ...
            <div class="relative">
              <input type="text" v-model="form.name" required placeholder="John Doe" class="w-full bg-gray-50 border-2 border-gray-100 p-4 pl-12 rounded-2xl outline-none focus:border-blue-600 focus:bg-white font-bold text-gray-900 transition-all">
              <svg class="w-5 h-5 absolute left-4 top-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
            </div>
          </div>
          ...
          <div>
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Create Password</label>
            <div class="relative">
              <input :type="showPassword ? 'text' : 'password'" v-model="form.password" required minlength="6" placeholder="••••••••" class="w-full bg-gray-50 border-2 border-gray-100 p-4 pl-12 pr-12 rounded-2xl outline-none focus:border-blue-600 focus:bg-white font-bold text-gray-900 transition-all">
       ...
       <div class="relative z-10 text-center max-w-md">
          <div class="bg-white p-6 rounded-[2rem] shadow-xl border border-gray-100 mb-8 transform -rotate-3 hover:rotate-0 transition-transform duration-500">
             <div class="flex items-center gap-4 mb-4">
                <div class="w-12 h-12 bg-green-100 text-green-600 rounded-xl flex items-center justify-center text-xl font-black">📈</div>
                <div class="text-left">
                   <p class="font-black text-gray-900">Top 5% Learner</p>
                   <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">Global Ranking</p>
                </div>
             </div>
             <div class="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                <div class="bg-gradient-to-r from-green-400 to-blue-500 w-3/4 h-full"></div>
             </div>
          </div>
          
          <h2 class="text-3xl font-black text-gray-900 tracking-tight mb-4">Unlock your potential.</h2>
          <p class="text-gray-500 font-medium leading-relaxed">Join a network of top-tier students. Access elite materials, take mock exams, and build your digital portfolio.</p>
       </div>
    </div>
...
const handleRegister = async () => {
  ...
  message.value = '';
  try {
    const response = await api.registerUser(form.value);
    isSuccess.value = true;
    message.value = "Account created securely! Redirecting to login...";
    ...
    message.value = err.response?.data?.message || "Registration failed. Please check your details.";
  } finally {
    loading.value = false;
  }
};
